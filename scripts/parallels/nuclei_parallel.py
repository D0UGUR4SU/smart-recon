...
def parallel():
    os.system('rm -rf ./docker/data/'+target+'/temp/nuclei_parallel.log')
    with open ('./docker/data/'+target+'/temp/nuclei_parallel.log','a') as file:
        for sis in dic_sistemas:
            file.write('python3 ./scripts/parsers/parse_nuclei.py '+target+' '+sis+' '+dic_sistemas[sis][0]+' '+dic_sistemas[sis][1]+' '+dic_sistemas[sis][2]+'\n')
