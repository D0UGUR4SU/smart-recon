				...
				if(x['_source']['server.port'] not in list_serv):
					...
					if(x['_source']['network.protocol'] in servicos):
						with open ('./docker/data/'+target+'/temp/hydra_parallel.log','a') as file:
            						file.write('python3 ./scripts/parsers/parse_hydra.py '+target+' '+i+' '+x['_source']['server.port']+' '+x['_source']['network.protocol']+'\n')
...
def main():
	os.system('rm -rf ./docker/data/'+target+'/temp/hydra_parallel.log')
	consulta()
	parallel() 
