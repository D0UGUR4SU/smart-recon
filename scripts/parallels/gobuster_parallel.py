...
def parallel():
    os.system('rm -rf ./docker/data/'+target+'/temp/gobuster_parallel.log')
    with open ('./docker/data/'+target+'/temp/gobuster_parallel.log','a') as file:
        for sis in dic_sistemas:
            file.write('python3 ./scripts/parsers/parse_gobuster.py '+target+' '+dic_sistemas[sis][0]+' '+dic_sistemas[sis][1]+' '+sis+' '+dic_sistemas[sis][3]+' '+str(dic_sistemas[sis][2])+'\n')
    print("[+] PROCESSANDO GOBUSTER \n")
    os.system('cat ./docker/data/'+target+'/temp/gobuster_parallel.log | parallel -u')
def main():
   consulta()
   parallel() 
