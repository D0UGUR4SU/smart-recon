...
def rdap_ip(ip):
    try:
        consulta1 = subprocess.check_output('docker run --rm --name '+container_name+' -v ./docker/data/'+target+':/data kali-tools:2.0 rdap '+ip+' --json || true', shell=True) 
...
def parse():
    with open ('./docker/data/'+target+'/temp/'+saida) as json_file:
