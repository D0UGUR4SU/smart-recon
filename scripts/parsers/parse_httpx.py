...
def executa(url):
    result = subprocess.check_output(f'docker run --rm --name {container_name} -v ./docker/data/{target}/temp:/data kali-tools:2.0 /bin/bash -lc "echo {url} | httpx --silent" || true', shell=True)
    return(result)
